...
def shell_volume_2d(cfg, root3: np.ndarray, tip3: np.ndarray, holes: list[dict], wall: float,
                    n_stations: int = 61) -> dict:
    ...
    return {"wall_mm": wall, "outer_mm3": v_outer, "cavity_mm3": v_cavity, "pillars_mm3": v_pillars,
            "bores_mm3": v_bores, "body_mm3": v_body, "mass_g": v_body / 1000.0 * cfg.density_g_cm3}
...
def build_report(b, exact: bool = True) -> dict:
    ...
    rep["airfoil"] = {
        "name": foil.name, "frame": foil.frame.describe() if foil.frame else "?",
        "te_cut_x_c": foil.x_te(), "te_gap_c": foil.te_gap(),
        "te_gap_root_mm": foil.te_gap() * cfg.root_chord_mm, "te_gap_tip_mm": foil.te_gap() * cfg.tip_chord_mm,
        "physical_root_length_mm": foil.x_te() * cfg.root_chord_mm,
        "physical_tip_length_mm": foil.x_te() * cfg.tip_chord_mm,
        "max_thickness_c": tm, "max_thickness_x_c": xm,
        "max_thickness_root_mm": tm * cfg.root_chord_mm, "max_thickness_tip_mm": tm * cfg.tip_chord_mm,
        "section_area_c2": foil.area(),
    }
    pf = cfg.planform()
    rep["planform"] = {"area_cm2": pf.planform_area() / 100.0, "taper": cfg.tip_chord_mm / cfg.root_chord_mm,
                       "tip_le_offset_mm": pf.station(1).x_le, "bbox": geo.bbox(b.body)}
    rep["holes"] = [{k: (float(v) if isinstance(v, (np.floating, float)) else v) for k, v in h.items()} for h in b.holes]
    if exact:
        v_body, a_body = geo.volume(b.body), geo.area(b.body)
        v_outer, a_outer = geo.volume(b.outer), geo.area(b.outer)
        rep["exact"] = {"body_volume_cm3": v_body / 1000, "body_area_cm2": a_body / 100,
                        "outer_volume_cm3": v_outer / 1000, "wetted_area_cm2": a_outer / 100,
                        "mass_g": v_body / 1000 * cfg.density_g_cm3,
                        "solid_mass_g": v_outer / 1000 * cfg.density_g_cm3}
    rep["wall_table"] = wall_table(cfg, b.root3, b.tip3, b.holes, rep.get("exact", {}).get("body_volume_cm3", None) and
                                   rep["exact"]["body_volume_cm3"] * 1000)
    return rep
...
def format_report(rep: dict) -> str:
    ...
    L.append("=" * 78)
    L.append(f"planform : root {cfg['root_chord_mm']:g} mm, tip {cfg['tip_chord_mm']:g} mm, span {cfg['span_mm']:g} mm, "
             f"LE sweep {cfg['le_sweep_deg']:g}°, washout {cfg['washout_deg']:g}°, dihedral {cfg['dihedral_deg']:g}°")
    L.append(f"           area {p['area_cm2']:.1f} cm², taper {p['taper']:.2f}, tip LE {p['tip_le_offset_mm']:.1f} mm aft of root LE")
    bb = p["bbox"]
    L.append(f"bbox     : x {bb['xmin']:.1f}..{bb['xmax']:.1f}  y {bb['ymin']:.1f}..{bb['ymax']:.1f}  z {bb['zmin']:.1f}..{bb['zmax']:.1f} mm")
    L.append(f"airfoil  : TE cut at {a['te_cut_x_c']:.3f} c -> physical length root {a['physical_root_length_mm']:.2f} mm, "
             f"tip {a['physical_tip_length_mm']:.2f} mm")
    L.append(f"           TE thickness root {a['te_gap_root_mm']:.2f} mm, tip {a['te_gap_tip_mm']:.2f} mm  "
             f"({'!! thin for FDM; consider a larger te_truncate_pct' if a['te_gap_tip_mm'] < 0.8 else 'ok'})")
    L.append(f"           max thickness {a['max_thickness_c'] * 100:.1f} % c at {a['max_thickness_x_c'] * 100:.0f} % c "
             f"-> {a['max_thickness_root_mm']:.1f} mm root, {a['max_thickness_tip_mm']:.1f} mm tip")
    L.append(f"checks   : outer valid {rep['checks'].get('outer_valid')}, cavity valid {rep['checks'].get('cavity_valid', '-')}, "
             f"body valid {rep['checks'].get('body_valid')}, loft-vs-Simpson {rep['checks'].get('simpson_rel_err', 0):.2%}")
    L.append("holes    : (spanwise from the root face, depth %g mm, boss wall %g mm, validated min wall %g mm)" % (
        cfg["hole_depth_mm"], cfg["boss_wall_mm"], cfg["hole_min_wall_mm"]))
    for h in rep["holes"]:
        L.append(f"           {h['name']:>6}: {h['x_pct']:5.1f} % c -> x={h['x']:6.2f} z={h['z']:5.2f} mm, ⌀{h['d_mm']:g}, "
                 f"min clearance {h['min_clearance']:+.2f} mm (at y={h['at_y']:.1f})")
    if "exact" in rep:
        e = rep["exact"]
        L.append(f"exact    : body volume {e['body_volume_cm3']:.2f} cm³, surface {e['body_area_cm2']:.1f} cm², "
                 f"wetted (outer) {e['wetted_area_cm2']:.1f} cm², solid-equivalent volume {e['outer_volume_cm3']:.1f} cm³")
        L.append(f"MASS     : {e['mass_g']:.1f} g at wall {cfg['wall_thickness_mm']:g} mm, TPU {cfg['density_g_cm3']:g} g/cm³ "
